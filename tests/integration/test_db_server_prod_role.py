...
HISTOGRAMS = [{
    'bin_values': [
        9, 9, 15, 8, 10, 18, 18, 25, 26, 28, 36, 35, 36, 34, 42, 33, 28, 37, 30, 41, 27, 21, 22, 33,
        41, 32, 34, 36, 37, 36, 36, 38, 32, 32, 36, 40, 29, 40, 42, 35, 46, 38, 41, 37, 50, 41, 52,
        67, 70, 58, 70, 86, 95, 94, 161, 174, 201, 154, 171, 190, 268, 343, 436, 453, 462, 416, 395,
        301, 230, 200, 159, 113, 80, 74, 57, 41, 47, 46, 42, 48, 38, 39, 39, 34, 37, 37, 24, 35, 30,
        20, 29, 21, 23, 31, 31, 30, 24, 34, 25, 37
    ],
    ...
    'bin_values': [
        133, 97, 41, 194, 240, 49, 229, 230, 200, 54, 138, 106, 262, 276, 87, 125, 89, 78, 216, 205,
        277, 206, 211, 223, 249, 138, 50, 218, 233, 104, 296, 170, 219, 298, 77, 89, 155, 277, 200,
        282, 64, 196, 251, 252, 55, 89, 70, 283, 138, 14, 138, 206, 116, 196, 45, 161, 107, 131,
        152, 186, 85, 178, 96, 54, 130, 221, 291, 63, 34, 89, 215, 135, 84, 117, 268, 206, 19, 179,
        279, 9, 142, 241, 282, 209, 11, 236, 232, 77, 250, 299, 43, 133, 91, 262, 65, 220, 243, 133
    ],
...
class TestDBServerProdRole:
    ...
    @staticmethod
    def _get_updated_histo(old_histo: Histogram, newer_histo_values: Histogram) -> Histogram:
        updated_histo = copy.deepcopy(old_histo)

        bin_values = [
            b1 + b2 for b1, b2 in zip(old_histo['bin_values'], newer_histo_values['bin_values'])
        ]
    ...
    def test_histo(db_rc: RestClient) -> None:
        ...
        def assert_get(histo: Histogram) -> None:
            get_body = {'database': 'test_histograms', 'collection': 'TEST', 'name': histo['name']}
        ...
        for orignial_histo in histograms:
            ...
            # GET
            assert_get(TestDBServerProdRole._get_updated_histo(orignial_histo, newer_histo))
    ...
    def test_file(db_rc: RestClient) -> None:
        ...
        post_body_1 = {'database': 'test_histograms', 'collection': collection_name, 'files': files}
        ...
        # 2. POST again with no update flag
        post_body_2 = {'database': 'test_histograms', 'collection': collection_name, 'files': files}
        with pytest.raises(requests.exceptions.HTTPError) as e:
